...
from helper import DataAnalyzer, BrazilMapPlotter
import os
...
with st.sidebar:
    ...
    with col1:
        st.write(' ')
        st.write("**Fitto Martcellindo**", width=True)
    with col2:
        st.image("https://raw.githubusercontent.com/Fitomarcelindo/DataAnalyst-Dicoding/main/foto_KemjaHitam.jpg", width=100)
...
# Placeholder classes for functionality
class DataAnalyzer:
    def __init__(self, df):
        self.df = df

    def create_daily_orders_df(self):
        return self.df.groupby(self.df["order_approved_at"].dt.date).size().reset_index(name="order_count")

    def create_sum_spend_df(self):
        return self.df.groupby(self.df["order_approved_at"].dt.date)["payment_value"].sum().reset_index(name="total_spend")

    def create_sum_order_items_df(self):
        return self.df.groupby("product_category_name_english")["order_item_id"].count().reset_index(name="product_count")

    def review_score_df(self):
        review_score = self.df["review_score"].value_counts()
        return review_score, review_score.idxmax()

    def create_bystate_df(self):
        state = self.df["customer_state"].value_counts()
        return state, state.idxmax()

    def create_order_status(self):
        status = self.df["order_status"].value_counts()
        return status, status.idxmax()

class BrazilMapPlotter:
    def __init__(self, data, plt, st):
        self.data = data
        self.plt = plt
        self.st = st

    def plot(self):
        self.st.image("https://raw.githubusercontent.com/Fitomarcelindo/DataAnalyst-Dicoding/main/MapBrazil.png", caption="Geolocation Distribution", use_column_width=True)

function = DataAnalyzer(main_df)
map_plot = BrazilMapPlotter(data, plt, st)
...
with col2:
    total_revenue = daily_orders_df["order_count"].sum()
...
with tab1:
    most_common_state = state.idxmax()
    st.markdown(f"Most Common State: **{most_common_state}**")

    fig, ax = plt.subplots(figsize=(12, 6))
    sns.barplot(x=state.index,
                y=state.values, 
                palette="viridis")
...
st.caption('Copyright (C) Fitto Martcellindo 2024')
